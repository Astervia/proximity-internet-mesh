...
| Bluetooth RFCOMM backend      | Supported | Sidecar       |
...
- Bluetooth RFCOMM uses BlueZ RFCOMM sockets and `bluetoothctl devices Paired` for outbound paired-device scans when `[bluetooth_rfcomm].enabled = true`.
...
- Bluetooth RFCOMM is handled by the macOS UI sidecar rather than the Rust daemon.
...
- [../architecture/transports/bluetooth.md](../architecture/transports/bluetooth.md) — Bluetooth PAN and RFCOMM backend details.
