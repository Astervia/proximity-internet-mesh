...
Even though the code contains `IpRequest`, `IpAssign`, and `IpPool`, the daemon currently expects an explicit `interface.mesh_ip` and will not start with `mesh_ip = "auto"`.
