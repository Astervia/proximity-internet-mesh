...
## 2024-05-18 - SQLite Prepared Statement Caching Optimization

**Learning:** In crates like `pim-daemon` and `pim-messaging`, repeated database interactions were using `conn.prepare()` to re-compile static SQL queries on every invocation. This introduces unnecessary parsing and compilation latency for frequently executed queries (like looking up peers or inserting messages) in an application sensitive to performance.
**Action:** Use `rusqlite::Connection::prepare_cached()` instead of `prepare()` for static queries to reuse the prepared statement plan and avoid redundant statement recompilation overhead.
