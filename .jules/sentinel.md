...
## 2026-05-18 - [Prevent Argument Injection in Network Interface Handling]
**Vulnerability:** External utilities like `ip`, `ifconfig`, `dhclient`, and `dnsmasq` were being invoked with user-controlled or dynamically discovered network interface names without prior sanitization. While `std::process::Command` prevents shell injection, it is still vulnerable to argument injection if an attacker can craft a network interface name starting with `-` or containing unexpected characters to manipulate the utility's behavior.
**Learning:** Relying solely on `std::process::Command`'s shell avoidance is insufficient when passing untrusted input as an argument to an external command that parses flags.
**Prevention:** Always validate network interface names against a strict allowlist of expected characters (alphanumeric, hyphens, underscores, periods, and colons) and length constraints before passing them to external command utilities.
