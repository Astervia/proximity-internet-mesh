...
## 2026-05-25 - Prevent command injection in Bluetooth module via interface names
**Vulnerability:** External command execution functions (e.g. `start_dnsmasq`, `start_dhclient`, `discover_neighbor_targets_impl`, `start_forwarding_impl`, `configure_nap_bridge_impl`) in `crates/pim-bluetooth/src/platform_impl.rs` were directly using interface names passed to `std::process::Command` without validation. If these names were sourced from an untrusted configuration, an attacker could inject arguments or sub-commands.
**Learning:** Even internal configuration elements such as interface names need strict validation before being interpolated into shell commands.
**Prevention:** Enforce rigorous input validation for all interface names using an allowlist approach (e.g., length limits and ensuring characters are alphanumeric, `-`, or `_`). A helper function `is_safe_interface_name` was introduced and consistently used before invoking `Command::new()`.
