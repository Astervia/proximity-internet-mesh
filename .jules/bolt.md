...
## 2026-05-30 - Rusqlite static statement caching optimization

**Learning:** `rusqlite`'s `Connection::execute` and `Connection::prepare` parse and compile the SQL statement on every invocation. For hot paths containing frequent inserts (e.g., messaging storage) or updates (e.g., peer directory lifecycle), this redundant SQLite compilation creates measurable CPU and latency overhead.
**Action:** Throughout the repository, prefer `conn.prepare_cached("SQL...")?.execute(params![...])` over `conn.execute("SQL...", params![...])` for static SQL queries to leverage the built-in statement cache (LRU size 16) and minimize execution latency in performance-sensitive paths.
