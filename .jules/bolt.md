...
## 2026-05-18 - Prepare cached statement
**Learning:** Use conn.prepare_cached() instead of conn.prepare() for SQL statements that are frequently executed.
**Action:** Replaced conn.prepare() with conn.prepare_cached() in peer_directory.rs and storage.rs to leverage SQLite statement caching, improving execution latency on performance-sensitive paths.
