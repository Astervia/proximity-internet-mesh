...
│   ├── pim-transport/          # Wi-Fi Direct transport + mock
...
│
├── pim-cli/                    # CLI binary (`pim up`, `pim down`, `pim status`)
│
└── tests/
...
User-facing CLI built with clap's derive API. Thin client that communicates with `pim-daemon` over a Unix socket. See [cli.md](cli.md) for full design, subcommand definitions, and output examples.
