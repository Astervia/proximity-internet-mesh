...
## 2026-05-21 - [Prevent Command Injection via Interface Names]
**Vulnerability:** The `pim-bluetooth` module executes standard commands (`ip`, `ifconfig`, `arp`, `iptables`, `bt-network`, `dnsmasq`, `dhclient`) using user-provided or dynamically obtained strings (e.g. interface names or bridge names) without proper validation. This exposes the system to command injection vulnerabilities, where a carefully crafted interface name could cause malicious commands to be executed with elevated privileges.
**Learning:** Shelling out to external commands and passing dynamic arguments requires rigorous input validation to ensure the arguments cannot be manipulated to alter the command's meaning or to introduce new commands (argument injection).
**Prevention:** Always validate inputs that are passed as arguments to external command utilities. Specifically for network interfaces, enforce structural validity using a function like `is_safe_interface_name` to restrict names to a safe subset of characters (e.g., alphanumeric, hyphens, underscores) and length limits before passing them to `Command::new`.
