...
## 2026-06-01 - [Prevent Command/Argument Injection via Interface Names]
**Vulnerability:** In `pim-bluetooth`, network interface and bridge names obtained from configuration or sysfs were directly interpolated into commands passed to `std::process::Command::new` (like `ip`, `ifconfig`, `dnsmasq`, `dhclient`, `arp`). While `Command::new` prevents traditional shell injection, it does not prevent argument injection if an attacker can manipulate the interface name to include leading hyphens (e.g., `-o`) or inject malformed data.
**Learning:** Relying on `std::process::Command` to mitigate shell injection is insufficient when dealing with untrusted strings that are used as arguments. A maliciously crafted interface name starting with a hyphen can be interpreted as a command-line flag by the target utility, potentially leading to arbitrary file reads, writes, or execution.
**Prevention:** Always validate and sanitize untrusted input using strict allowlists (e.g., `is_safe_interface_name` allowing only alphanumeric characters, `.`, `:`, `-`, `_` and explicitly rejecting names starting with `-` or exceeding 15 characters) before passing it as arguments to external command utilities.
