...
## 2026-05-14 - [Command Injection Prevention via Interface Name Validation]
**Vulnerability:** In `pim-bluetooth`, external command utilities (like `ifconfig`, `ip`, and `arp`) were invoked with dynamically provided `interface` string arguments without validation. This created a potential command/argument injection vulnerability where malformed interface names (e.g. `eth0; rm -rf /`) could execute arbitrary commands.
**Learning:** Shell commands that interpolate dynamic configuration or network state must aggressively validate those strings. Even when `Command::new().arg()` escapes basic shell metacharacters, arguments that start with `-` or contain unexpected characters can manipulate the target utility into executing unsafe actions (argument injection).
**Prevention:** Implement strict allowlist validation for identifiers passed to external commands. A helper like `is_safe_interface_name` should enforce constraints (e.g., alphanumeric, hyphens, underscores, dots) and reasonable length limits before the value is used as an argument.
