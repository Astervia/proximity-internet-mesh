...
## 2026-05-27 - [Interface Validation Must Allow Dots and Colons]
**Vulnerability:** External networking utilities (`ip`, `dhclient`, etc.) can be manipulated via command/argument injection if unsanitized interface names (e.g., `-e`) are passed from user input.
**Learning:** While strict validation (alphanumeric, `-`, `_`) prevents injection, it can accidentally break valid Linux networking setups like VLANs (`eth0.100`) or interface aliases (`eth0:0`).
**Prevention:** When writing custom validation functions (like `is_safe_interface_name`), explicitly permit periods (`.`) and colons (`:`) in addition to standard safe characters to ensure compatibility with complex network topologies without sacrificing security.
