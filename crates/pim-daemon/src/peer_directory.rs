...
impl PeerDirectoryService {
    // Optimization: We use `conn.prepare_cached()` instead of `conn.execute()`
    // for static SQL queries throughout this service to leverage statement caching
    // and minimize execution latency in performance-sensitive paths.
